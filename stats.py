...
puzzle_id= np.arange(0, len(puzzle_f))
...
#list all the unique puzzle files, participants and sort them
...
df_main=df_main.groupby(['participantID', 'puzzle-file', 'run','attempt'])

#delete rows with solved as false
df_main=df_main.filter(lambda x: x['solved'].iloc[-1] == True)
df_main=df_main.groupby(['participantID', 'puzzle-file', 'run','attempt'])["solved"].count()
